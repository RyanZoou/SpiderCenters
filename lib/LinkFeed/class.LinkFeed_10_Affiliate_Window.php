...
class LinkFeed_10_Affiliate_Window
{
    ...
    function GetProgramByApi()
    {
        ...
        foreach ($ptsp_pgrm_r as $pv) {
            ...
            if ($this->isFull && !$objProgram->checkBaseProgramInfoExistsByIdinaff($this->info['NetworkID'], $idInAff, $this->info['crawlJobId'])) {
                ...
                $sem_tmp = '';
                $ppc_url = "https://ui.awin.com/awin/affiliate/{$this->info['APIKey2']}/merchant-profile-terms/{$idInAff}/ppc";
                $sem_res = $this->oLinkFeed->GetHttpResult($ppc_url, $request);
                $sem_res = $sem_res['content'];
                $sem_tmp = trim($this->oLinkFeed->ParseStringBy2Tag($sem_res, '<div class="table-responsive">', '</div>'));
                
                $SupportType = "";
                $spt_url = "https://ui.awin.com/awin/affiliate/{$this->info['APIKey2']}/merchant-profile-terms/{$idInAff}/affiliate";
                $spt_res = $this->oLinkFeed->GetHttpResult($spt_url, $request);
                $spt_res = $spt_res['content'];
                $spt_tmp = trim($this->oLinkFeed->ParseStringBy2Tag($spt_res, '<tbody id="termsContent">', '</tbody>'));
                $contact_type = $this->oLinkFeed->ParseStringBy2Tag($spt_tmp, array('question-promotionTypes','id="content">'), '</tr>');
                if (stripos($contact_type, 'tick.png')){
                	$SupportType .= "Content".EX_CATEGORY;
                }
                $coupon_type = $this->oLinkFeed->ParseStringBy2Tag($spt_tmp, array('question-promotionTypes','id="code">'), '</tr>');
                if (stripos($coupon_type, 'tick.png')){
                	$SupportType .= "Coupon".EX_CATEGORY;
                }
                if (stripos($contact_type, 'cross.png') && stripos($coupon_type, 'cross.png')){
                	echo "idinaff : {$idInAff} don't support Contact and Coupon".PHP_EOL;
                }
                ...
                $arr_prgm[$idInAff] += array(
            		...
            		"SEMPolicyExt" => addslashes($sem_tmp),
	...
	function getTransactionFromAff($start_date, $end_date)
	{
		...
		$api_url = "https://api.awin.com/publishers/{$this->info['APIKey2']}/transactions/?startDate={BEGIN_DATE}T00%3A00%3A00&endDate={END_DATE}T23%3A59%3A59&timezone=Canada/Pacific&accessToken={$this->info['APIKey4']}";
		$request = array("AccountSiteID" => $this->info["AccountSiteID"],"method" => "get");
